...
Note that the file isn't intended to be read on github (or briefly) as
their render of org-mode is incomplete (the following results for
instance). You can read it in online in different formats on [[https://7d.nz/A-Python-interactive-guide-on-Org-babel.html][7d.nz]].
...
type it holds can change during the execution of the program.
...
for n in range(0, 11):
    x=2<<n
    print('{4:2d} {0:5d} {1:7d} {2:4d}{3:12d}'.format(x,x**2,int(x**2/1024),x**3,n))
...
* TODO Magic methods
see [[./magicmethods.org][magicmethods.org]]
