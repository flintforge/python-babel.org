#+TITLE: A Python crash course on Org-babel
#+Date: [2020-12-18 ven.]
#+Author: Philippe Estival
#+email: [[mailto:phil.estival@free.fr]]

* Intro
To start on the right foot, one can read the begining
of python.el (Python's flying circus support for Emacs),
which provides =python-mode=, and relies on =comint=.

Beginners can read the tutorial and the guide
https://docs.python.org/3/using
https://docs.python.org/3.6/tutorial

The present guide is a quick introduction, turning
mostly into a collection of code snippets.

It features some of interactions =org-babel= provides
over code source blocks.
It's written with Emacs 26.3 and Org-mode 9.3.6
...
#+call:emacs-version
#+results:
: GNU Emacs 26.3 (build 1, x86_64-pc-linux-gnu, X toolkit, Xaw scroll bars)
:  of 2020-10-22
...
is /on/ the code blocks.
   ...
   side-note : the proper way to write a call is with two
   parentheses and eventually a named argument, for
   instance, #+CALL: factorial(n=5), but see, I found a bug,
   (or is it really one ?) and decided to go with it.
...
#+begin_src python :results output
  def greetings(name):
     """ And here we can describe what the function does
     """
     print ("Hello %s" % name)
...
unless we enter session mode.
 ...
 expression, as in an interactive shell.
  ...
  [2020-12-19 sam. 17:35] Things could be different now in org 9.3
...
non-session output :
#+RESULTS:
: ok <__main__.aClass object at 0x7f6a9f80c978>
: None
: <class '__main__.aClass'> __del__ <__main__.aClass object at 0x7f6a9f80c978>

session output in
#+RESULTS:
: <class '__main__.aClass'> __del__ <__main__.aClass object at 0x7f9a53e1b4a8>
: ok <__main__.aClass object at 0x7f9a53e1b8d0>
: None



** Fonction /single dispatch/ générique
Polymorphisme à la Java à l'aide de _décorateurs_
...
#+Begin_src python :results output
  ...
  @F.register(float)
  def _(arg):
      return "pour un nombre"

  class C: pass

  @F.register(C)
  def _(arg):
      return "pour un objet C"
...
: ('par défaut', 'par défaut', 'pour un nombre', 'pour un objet C')
: dict_keys([<class 'float'>, <class 'int'>, <class 'object'>, <class '__main__.C'>])
...
* Installation of further Python versions

In further versions of Python are introduced
new language features.
For instance as of 3.6
was introduced the matrix multiplicatioin operator (@).
...
*** TODO Configuration des touches
*** TODO sauvegarde du configurateur d'objet
*** TODO FrameBSP
#+Begin_src python
...
t.start()
# l'interpreteur se poursuit
# et on peut lui indiquer les nouvelles valeurs
# lue par la simulation
...
clearcolor=(0,0,0)
def justdoit(): # call Ion.justdoit() from pyshell
    global clearcolor
    clearcolor=(0,1,0)
...
def start():
    print ('now what ?\n')
    sys.exit(run())
# yes, it's already out unless a root tk is still flying around
#+end_src
